...
void configureToolbar(QToolBar *toolbar, QSpdLog *logView, std::shared_ptr<spdlog::logger> logger) {
    ...
    generateAction->connect(generateAction, &QAction::triggered, [logger](bool) {
        ...
        for (int i = 0; i < 10; ++i)
            logger->log(static_cast<spdlog::level::level_enum>(rand() % spdlog::level::off),
                        "Message {}",
                        i);
    });

    generateMultipleAction
        ->connect(generateMultipleAction, &QAction::triggered, [logView, logger](bool) {
    ...
    clearAction->connect(clearAction, &QAction::triggered, [logView](bool) { logView->clear(); });
}
...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent) {
    ...
    this->setCentralWidget(m_spdlogWidget);
    ...
    m_spdlogWidget->registerToolbar(logToolbar);
    dynamic_cast<QWidget *>(logToolbar)->show();

    logger->sinks().push_back(m_spdlogWidget->sink());

    configureToolbar(m_toolbar, m_spdlogWidget, logger);
}
